...
return Application::configure(basePath: dirname(__DIR__))
    ...
    ->withMiddleware(function (Middleware $middleware): void {
        ...
        $middleware->alias([
            'auth'=> AuthenticateMiddleware::class, //pour le guard 'web'
            'authAdmin' => AuthenticateAdminMiddleware::class, //pour le guard 'admin'
            'auth.redirected' => RedirectedMiddleware::class, //redirection selon guard

        // =============== Middleware de rôle
            'role' => RoleMiddleware::class, //vérifie que l'utilisateur a un rôle

        // =============== Middlewares d'accès aux ressources
            'accessAgent'=> AgentAccessMiddleware::class, //autorise l'accès à un utilisateur de rôle 'agent'
        ...
        // $middleware->alias('this.estate.owner', EstateBelongsToOwner::class); //autorise l'accès à un bien seulement à l''owner' correspondant
            'thisServiceAgent'=> ServiceBelongsToAgent::class, //autorise l'accès à un service seulement à l''agent' correspondant
        ...
        // $middleware->alias('this.estate.service', ServiceHasThisEstateOwner::class); //autorise l'accès à un service seulement à l''owner' du bien correspondant
            'thisEstateAgent'=> EstateHasThisAgent::class, //autorise l'accès à un bien seulement à l''agent' correspondant
        ]);
    })
